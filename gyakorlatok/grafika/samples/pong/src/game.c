...
void update_game(Game* game, double time)
{
    ...
    if (game->ball.x - game->ball.radius < 50) {
		if(game->ball.y >= game->left_pad.position && game->ball.y < (game->left_pad.position + game->right_pad.size)){
    ...
    if (game->ball.x + game->ball.radius > game->width - 50) {
		if(game->ball.y >= game->right_pad.position && game->ball.y < (game->right_pad.position + game->right_pad.size)){
